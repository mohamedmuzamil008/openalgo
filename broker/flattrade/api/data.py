...
class BrokerData:
    ...
    def get_history_intraday(self, symbol: str, exchange: str, interval: str, start_date: str, end_date: str, start_time: str, end_time: str) -> pd.DataFrame:
        """
        Get historical data for given symbol
        Args:
            symbol: Trading symbol
        ...
        try:
            ...
            # Convert dates to epoch timestamps
            start_ts = int(datetime.strptime(start_date + " " + start_time, '%Y-%m-%d %H:%M:%S').timestamp())
            end_ts = int(datetime.strptime(end_date + " " + end_time, '%Y-%m-%d %H:%M:%S').timestamp())
